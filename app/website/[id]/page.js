...
export default function WebsiteDetailContent() {
  ...
  const groupSectionsByPage = (data) => {
    const pages = {};
    ...
    Object.entries(data).forEach(([pageKey, pageValue]) => {
      if (pageValue && typeof pageValue === "object" && !Array.isArray(pageValue)) {
        pages[pageKey] = {
          name: pageKey.replace(/([A-Z])/g, " $1").replace(/page/gi, " Page").trim(),
          sections: Object.entries(pageValue).map(([sectionKey, sectionValue]) => ({
            key: sectionKey,
  ...
  const pageGroups = groupSectionsByPage(website.allData || {});
